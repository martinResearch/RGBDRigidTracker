...
int main(int argc, char **argv)
{
	...
	int icpMaxIter = std::stoi(argv[3]);//10
	float voxelGridSize = std::stof(argv[4]);//0.05;
	float minSampleDistance = std::stof(argv[5]);//0.1;
	float maxCorrespondenceDistance = std::stof(argv[6]);//0.05;
	float ICPMaxCorrespondenceDistance = std::stof(argv[7]);//0.03;
	int initMaxIter = std::stoi(argv[8]);//50;
	float RadiusSearch = std::stof(argv[9]);//0.3;
	float featureRadius = std::stof(argv[10]);//0.2;

	if (argc != 11)
	{
		printf("Usage: surfaceAlign.exe model.obj pcdSequence.txt icpMaxIter voxelGridSize minSampleDistance maxCorrespondenceDistance ICPMaxCorrespondenceDistance initMaxIter RadiusSearch featureRadius\n");
		return (-1);
	}
	printf("Options:\n", icpMaxIter);
	printf("  icpMaxIter=%d\n", icpMaxIter);
	...
	pcl::io::savePLYFileBinary("reducedScene0.ply", *reducedCloud);
	...
	pcl::io::savePLYFileBinary("initialFit.ply", *cloudFinalPtr);
	...
	PointCloudWithNormals::Ptr modelPointCloudWithNormalsPtr(new PointCloudWithNormals);
	pcl::concatenateFields(*modelPointCloudPtr, *normals, *modelPointCloudWithNormalsPtr);
	icp.setInputTarget(modelPointCloudWithNormalsPtr);

	PointCloudWithNormals::Ptr alignedScene(new PointCloudWithNormals);
	PointCloud::Ptr alignedModel(new PointCloud);
	PointCloudWithNormals::Ptr sceneWithNormals(new PointCloudWithNormals);
	...
	while (!viewer.wasStopped())
	{
		...
		if (next_frame)
		{
			...
			scene_norm_est.setInputCloud(scenesPtr[idFrame]);
			scene_norm_est.compute(*scene_normals);
			pcl::concatenateFields(*scenesPtr[idFrame], *scene_normals, *sceneWithNormals);
			

			icp.setInputSource(sceneWithNormals);
			...
			pcl::io::savePLYFileBinary("aligned" + std::to_string(idFrame) + ".ply", *alignedModel);
			idFrame++;
